...
const NewLogin = () => {
  ...
  useEffect(() => {
    // Check if jwtString is present in localStorage
    const jwtString = localStorage.getItem("jwtString");
    if (jwtString) {
      // Redirect to /explore if jwtString exists
      navigate("/explore");
    }
  }, [navigate]); 
  ...
  const createEphemeralKeyPairandLogin = () => {
  ...
  return (
    <main>
      ...
      <div className={Styles.signinbutton} onClick={createEphemeralKeyPairandLogin}>
        <img src={googleicon} className={Styles.googleicon} alt="" />
        <p>Sign In With Google (ZK Login)</p>
      </div>
